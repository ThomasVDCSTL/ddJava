...
public class DatabaseCRUD {
    ...
    public int createPlateau(Characters player)throws SQLException{
        PreparedStatement pstmt= mydb.prepareStatement("INSERT INTO plateau (gobelin,sorcier,dragon,massue,epee,eclair,boule_de_feu,potion_m,potion_g,vide) VALUES (?,?,?,?,?,?,?,?,?,?)",Statement.RETURN_GENERATED_KEYS);
    ...
    public void updatePlateau(Characters player)throws SQLException{
        PreparedStatement pstmt= mydb.prepareStatement("UPDATE plateau SET gobelin=?,sorcier=?,dragon=?,massue=?,epee=?,eclair=?,boule_de_feu=?,potion_m=?,potion_g=?,vide=? WHERE plateau_id=?");
        String[] tileList=getTileList(player.getPlateau());
        for (int i = 0; i < 10; i++) {
            pstmt.setString(i+1,tileList[i]);
        }
